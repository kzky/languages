...
class VAE(object):
    ...
    def _MLP(self, x, in_dim, out_dim, scope, activation=True):
        ...
        with scope: 
            # Parameter
            W = tf.Variable(
                tf.truncated_normal(shape=[in_dim, out_dim])*0.001)
            b = tf.Variable(
                tf.truncated_normal(shape=[out_dim])*0.001)
    ...
    def _compute_stats(self, h):
        ...
        log_sigma_square = self._MLP(h, self._mid_dim, self._latent_dim,
                                         enc_scope, False)
        #self._log_sigma_square = tf.clip_by_value(log_sigma_square, -1000, 100)
