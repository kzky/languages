...
if __name__ == '__main__':
    ...
    ## box
    scale = 4
    bias = np.asarray([-7.5, 0, 0])
    np_vertices, np_triangles, np_vertex_colors = create_box(scale=scale, bias=bias)
    ...
    tmesh.n_specular = args.n_specular
    tmesh.Ks = args.Ks
    tmesh.material_type = MaterialType.REFLECTION
    tmesh.material_type, tmesh.ior = MaterialType.REFLECTION_AND_REFRACTION, 1.6
    tmeshes.append(tmesh)
    ## sphere
    scale = 1.5
    bias = np.asarray([3, 0, -3])
    np_vertices, np_triangles, np_vertex_colors = create_sphere(scale=scale, bias=bias)
    vertices2 = torch.from_numpy(np_vertices).to(device)
    triangles2 = torch.from_numpy(np_triangles).to(device)
    vertex_colors2 = torch.from_numpy(np_vertex_colors).to(device)
    vertices_ptr = vertices2.data_ptr()
    triangles_ptr = triangles2.data_ptr()
    vertex_colors_ptr = vertex_colors2.data_ptr()
    tmesh = TriangleMesh(triangles2.shape[0], vertices2.shape[0],
                         triangles_ptr, vertices_ptr, vertex_colors_ptr,
                         MaterialType.DIFFUSE)
    tmesh.n_specular = args.n_specular
    tmesh.Ks = args.Ks
